...
@section('content')
    ...
    <x-form.auth-forms title="Регистрация" action="{{route('register.handle')}}"  method="POST">

        @csrf
        <x-form.text-input name="name" type="text" placeholder="Имя" requred="true" value="{{old('name')}}" :isError="$errors->has('name')" />
        ...
        <x-form.text-input name="email" type="email" placeholder="E-mail" requred="true" value="{{old('email')}}" :isError="$errors->has('email')" />
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                ...
                <div class="text-xxs md:text-xs"><a href="{{route('login')}}"
                        class="text-white hover:text-white/70 font-bold">Войти в аккаунт</a></div>
            </div>
        </x-slot:buttons>
